...
class ScPlayback(ScBase):
	...
	def __init__(self, pCTX, pRender, pWanem):
		...
		self.ptDef.insert(0, self.CreateTocuhDef("BtBack",     468,        29,         62, 42, self.BtHandler))
		self.ptDef.insert(1, self.CreateTocuhDef("BtPrev",     470,        95,         43, 90, self.BtHandler))
		self.ptDef.insert(2, self.CreateTocuhDef("BtNext",      65,        95,         43, 90, self.BtHandler))
		#self.ptDef.insert(3, self.CreateTocuhDef("BtAuto",     460,       268,         80, 50, self.BtHandler))
		self.ptDef.insert(4, self.CreateTocuhDef("BtStop",     370,       268,         80, 50, self.BtHandler))
		self.ptDef.insert(5, self.CreateTocuhDef("BtPlay",     280,       268,         80, 50, self.BtHandler))
		self.ptDef.insert(6, self.CreateTocuhDef("BtPause",    190,       268,         80, 50, self.BtHandler))
		self.ptDef.insert(7, self.CreateTocuhDef("BtRepeat",   100,       268,         80, 50, self.BtHandler))
	...
	def RenderPanel(self, panelIdx, isActive, isFocus = False, datPath = ""):
		...
		c = self.pRender.ConvRgb(0.31,0.2,0.1)
		self.pRender.fb.putstr(52+10     + offsetX,  84+10+12*2, "Modify", c, 1)
		self.pRender.fb.putstr(52+10+70  + offsetX,  84+10+12*2, "Time", c, 1)
		self.pRender.fb.putstr(52+10     + offsetX,  84+10+12*3, datMtime, c, 1)
		self.pRender.fb.putstr(52+10+70  + offsetX,  84+10+12*3,
							   self.seekManager.Conv2FormatedTime(dat["dps"], dat["duration"]), c, 1)
		self.pRender.fb.putstr(52+10     + offsetX,  84+10+12*4, "Memo", c, 1)		
		self.pRender.fb.putstr(52+10     + offsetX,  84+10+12*5, dat["memo"][0:17], c, 1)		
	...
	def UpdatePanel(self, vec, forceClear = False):
		...
		if (self.datPageIdx * 3 + self.datFocusIdx) == 0 and vec == -1:
			return
		if (self.datPageIdx * 3 + self.datFocusIdx) == (self.datNr - 1) and vec == 1:
			return
	...
	def RenderFocus(self, idx):
		...
		self.pRender.fb.draw.rect(c, Rect(48 + 128*idx,  84-4+90+4,   128, 4), 0)
	...
	def RenderToggleFocus(self, idx, isActivey):
		...
		self.pRender.fb.draw.rect(c, Rect(288 + xoffset,       264-2+50+2,   84,  2), 0)
	...
	def RenderSeekInfo(self):
		...
		self.pRender.fb.putstr(445, 240-21, self.seekManager.GetTotalFormatTime(), self.pRender.W, 1)
		...
		if self.state == self.STATE_PLAY:
			...
			if self.pCTX.tick % self.seekManager.updateInterval == 0:				
				...
				if (self.pCTX.tick % 15) == 0:
					self.RenderCurrentInfo("", self.dat[self.seekManager.seekFrame])
	...
	def UpdateSeekTime(self):		
		...
		self.pRender.fb.putstr(224, 219, self.seekManager.GetCurrentFormatTime(), self.pRender.W, 1)
	...
	def Start(self):
		...
		self.pRender.UpdateSubTitle("Dat Path : " + self.pCTX.currentReplayData)
		...
		self.pRender.fb.draw.rect(c, Rect(1, 240 - 12, self.pRender.xres-2, 1), 0)		
		...
		self.RenderCurrentInfo("STOP", 0)
		self.pRender.fb.putstr(12+54, 268+24+6, "msec", self.pRender.W, 1)
	...
	def RenderCurrentInfo(self, state = "", delay = -1):
		...
		if delay >= 0:
			self.pRender.fb.draw.rect(self.pRender.N, Rect(12, 268+24, 50, 16), 0)		
			self.pRender.fb.putstr(12, 268+24, "%04d" % delay, self.pRender.W, 2)
