...
class ScManualEx2(ScBase):
	...
	def __init__(self, pCTX, pRender, pWanem):
		...
		self.ptDef.insert(0,  self.CreateTocuhDef("BtMenu",        468, 29,  62,  42, self.BtHandler))
		self.ptDef.insert(1,  self.CreateTocuhDef("BtApply",       149, 255, 110, 48, self.BtHandler))
		self.ptDef.insert(2,  self.CreateTocuhDef("BtConnL",       346, 255, 80,  48, self.BtHandler))
		self.ptDef.insert(3,  self.CreateTocuhDef("BtConnR",       258, 255, 80,  48, self.BtHandler))
		self.ptDef.insert(4,  self.CreateTocuhDef("BtUp1L",    477+6, 158, 40,  26, self.BtHandler))
		self.ptDef.insert(5,  self.CreateTocuhDef("BtUp1R",    405+6, 158, 40,  26, self.BtHandler))
		self.ptDef.insert(6,  self.CreateTocuhDef("BtUp10L",   477+6, 158+30*1, 40, 26, self.BtHandler))
		self.ptDef.insert(7,  self.CreateTocuhDef("BtUp10R",   405+6, 158+30*1, 40, 26, self.BtHandler))
		self.ptDef.insert(8,  self.CreateTocuhDef("BtUp100L",  477+6, 158+30*2, 40, 26, self.BtHandler))
		self.ptDef.insert(9,  self.CreateTocuhDef("BtUp100R",  405+6, 158+30*2, 40, 26, self.BtHandler))
		self.ptDef.insert(10, self.CreateTocuhDef("BtDw1L",    368+6, 158, 40,  26, self.BtHandler))
		self.ptDef.insert(11, self.CreateTocuhDef("BtDw1R",    368-72+6, 158, 40,  26, self.BtHandler))
		self.ptDef.insert(12, self.CreateTocuhDef("BtDw10L",   368+6,    158+30*1, 40, 26, self.BtHandler))
		self.ptDef.insert(13, self.CreateTocuhDef("BtDw10R",   368-72+6, 158+30*1, 40, 26, self.BtHandler))
		self.ptDef.insert(14, self.CreateTocuhDef("BtDw100L",  368+6,    158+30*2, 40, 26, self.BtHandler))
		self.ptDef.insert(15, self.CreateTocuhDef("BtDw100R",  368-72+6, 158+30*2, 40, 26, self.BtHandler))

		self.ptDef.insert(16, self.CreateTocuhDef("BtEditBand",  248, 98,  52, 39, self.BtHandler))
		self.ptDef.insert(17, self.CreateTocuhDef("BtEditDelay", 248, 147, 52, 39, self.BtHandler))
		self.ptDef.insert(18, self.CreateTocuhDef("BtEditLoss",  248, 196, 52, 39, self.BtHandler))
	...
	def BtHandler(self, key):
		...
		if key == "BtMenu":
			...
			if self.isApply != 0:
				self.pWanem.EmuDisconnToggleMini(self.upBand, self.dwBand, self.upDelay, self.dwDelay, self.upLoss, self.dwLoss)
		elif key == "BtConnR":
			if self.isApply != 0:
				self.pWanem.EmuDisconnPushMini(self.upBand, self.dwBand, self.upDelay, self.dwDelay, self.upLoss, self.dwLoss)
		elif key == "BtApply":
	...
	def TouchUpHandler(self, x, y):
		...
		if self.isApply != 0:		
			self.pWanem.EmuDisconnReleaseMini(self.upBand, self.dwBand, self.upDelay, self.dwDelay, self.upLoss, self.dwLoss)		
		return
			
	def ApplyDirectParam(self):
		self.RenderWire(True)
		self.pWanem.DirectApplyEx(self.upBand, self.dwBand, self.upDelay, self.dwDelay, self.upLoss, self.dwLoss)
	...
	def UpdateUpParam(self, delta):
		...
		if self.currentEdit == 0:
			...
			if self.upBand < 32:
				self.upBand = 32
			self.RenderParamForm(0, "Up Band", "%04d" % self.upBand,  "kbps", True)			
			...
			if self.upDelay < 0:
				self.upDelay = 0
			self.RenderParamForm(0, "Up Delay", "%04d" % self.upDelay,  "msec", True)
	...
	def UpdateDwParam(self, delta):
		...
		if self.currentEdit == 0:
			...
			if self.dwBand < 32:
				self.dwBand = 32
			self.RenderParamForm(1, "Dw Band",  "%04d" % self.dwBand,  "kbps", True)				
			...
			if self.dwDelay < 0:
				self.dwDelay = 0
			self.RenderParamForm(1, "Dw Delay",  "%04d" % self.dwDelay,  "msec", True)
	...
	def RenderParamRowBt(self, idx, isActive = False):
		...
		if isActive == False:
			...
			self.pRender.fb.draw.rect(c, Rect(238 + 3,       84 + 50 * idx + 4,       2, 40), 0)
			self.pRender.fb.draw.rect(c, Rect(238 + 2 + 55 , 84 + 50 * idx + 4,       2, 40), 0)
			self.pRender.fb.draw.rect(c, Rect(238 + 3 ,      84 + 50 * idx + 4 + 38,  56, 2), 0)
		
		if isActive == True:
			c = self.pRender.ConvRgb(0.9,1.0,1.0);				
			self.pRender.fb.draw.rect(c, Rect(241 , 88 + 50 * idx,  56, 2), 0)
			self.pRender.fb.draw.rect(c, Rect(241 , 88 + 50 * idx + 38,  56, 2), 0)
			self.pRender.fb.draw.rect(c, Rect(241 , 88 + 50 * idx,  2, 40), 0)
			self.pRender.fb.draw.rect(c, Rect(241 + 54 , 88 + 50 * idx,  2, 40), 0)
		...
		if isActive == True:
			...
			self.pRender.fb.putstr(243 + 3,  90+ 50 * idx + 11,  "LINK", self.pRender.N, 2)
		else:
			self.pRender.fb.draw.rect(c, Rect(243 , 90 + 50 * idx - 2,  52, 35), 0)
			self.pRender.fb.putstr(243 + 3,  90+ 50 * idx + 11 - 2,  "EDIT", self.pRender.N, 2)
	...
	def RenderParamUI(self, idx):
		...
		self.pRender.fb.putstr(116*idx + 8+33+6, 152+30+1,   "10",  c, 2)
		self.pRender.fb.putstr(116*idx + 8+33,   152+30*2,   "100",  c, 2)

		c = self.pRender.ConvRgb(0.44,0.9,0.7);
		self.pRender.fb.draw.rect(c, Rect(116*idx + 8,    146+30*0,  30, 26), 0)
		self.pRender.fb.draw.rect(c, Rect(116*idx + 8+72, 146+30*0,  30, 26), 0)
		self.pRender.fb.draw.rect(c, Rect(116*idx + 8,    146+30*1,  30, 26), 0)
		self.pRender.fb.draw.rect(c, Rect(116*idx + 8+72, 146+30*1,  30, 26), 0)
		self.pRender.fb.draw.rect(c, Rect(116*idx + 8,    146+30*2,  30, 26), 0)
		self.pRender.fb.draw.rect(c, Rect(116*idx + 8+72, 146+30*2,  30, 26), 0)
	...
	def RenderParamForm(self, idx, label, value, unit, isParamOnly = False):
		...
		if isParamOnly == True:
			self.pRender.fb.draw.rect(self.pRender.N, Rect(116*idx+12, 112, 60, 18), 0)
		else:
			...
			self.pRender.fb.draw.rect(self.pRender.N, Rect(116*idx+8 + 34,  84, 62, 16), 0)
			self.pRender.fb.draw.rect(self.pRender.N, Rect(116*idx+18, 114, 82, 16), 0)
		...
		self.pRender.fb.putstr(116*idx + 8+10+58, 120, unit,     self.pRender.W, 1)
	...
	def Start(self):
		...
		self.RenderParamRow(0,"Up Band", "%04d" % self.upBand, "kbps","Dw Band", "%04d" % self.dwBand, "kbps", True,
							wanemModeLabel)
		self.RenderParamRow(1,"Up Delay","%04d" % self.upDelay,"msec","Dw Delay","%04d" % self.dwDelay,"msec", False)
		self.RenderParamRow(2,"Up Loss", "%4d"  % self.upLoss, "%", "Dw Loss",   "%4d"  % self.dwLoss, "%",    False)
		...
		self.pRender.fb.putstr(286-140, 80*3+64, "Disconnect Emulation", c, 1)
		c = self.pRender.ConvRgb(0.98,0.3,0.3)
		self.pRender.fb.draw.rect(c, Rect(286-140, 80*3 + 8 + 44, 80, 4), 0)
		self.pRender.fb.draw.rect(c, Rect(380-140, 80*3 + 8 + 44, 80, 4), 0)
