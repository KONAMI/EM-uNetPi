...
class ScRemoteApi(ScBase):
	...
	def ApplyApiCall(self, query):
		...
		self.pWanem.DirectUpdateEx2(self.upBand, self.dwBand, self.upDelay, self.dwDelay, upLoss, dwLoss)
	...
	def Start(self):
		...
		self.pRender.fb.draw.rect(c, Rect(342,    74, 1,  self.pRender.yres - 74), 0)		
		...
		self.pRender.UpdateSubTitle("API Endpoint >> " + socket.inet_ntoa(in_addr) + ":" + str(self.bindPort))
		...
		self.pWanem.DirectApplyEx2(self.upBand, self.dwBand, self.upDelay, self.dwDelay, self.upLoss, self.dwLoss)
	...
	def RenderParamOnly(self, idx, isToggle, up, down):
		if isToggle == 0:
			...
			self.pRender.fb.draw.rect(c, Rect(20+102+106+12, 86+22+idx*58, 50, 16), 0)		
			self.pRender.fb.draw.rect(c, Rect(20+102+12, 86+22+idx*58, 50, 16), 0)		
			c = self.pRender.ConvRgb(0.32,0.4,0.4)
			self.pRender.fb.putstr(20+102+12,  86+22+idx*58, "%04d" % up, c, 2)
			self.pRender.fb.putstr(20+102+106+12,  86+22+idx*58, "%04d" % down, c, 2)
		else:
			if up == 0:
				c = self.pRender.ConvRgb(0.32,0.1,1)
			else:
	...
	def RenderParamBar(self, idx, isToggle, label, up = 0, down = 0, unit = ""):
		...
		c = self.pRender.ConvRgb(0.32,0.1,1)
		self.pRender.fb.draw.rect(c, Rect(20+102,     86+12+idx*58, 100, 32), 0)
		self.pRender.fb.draw.rect(c, Rect(20+102+106, 86+12+idx*58, 100, 32), 0)
		...
		if isToggle == 0:
			c = self.pRender.ConvRgb(0.32,0.4,0.4)
			self.pRender.fb.putstr(20+102+70,  86+30+idx*58, unit, c, 1)
			self.pRender.fb.putstr(20+102+12,  86+22+idx*58, "%04d" % up, c, 2)
			self.pRender.fb.putstr(20+102+106+70,  86+30+idx*58, unit, c, 1)
			self.pRender.fb.putstr(20+102+106+12,  86+22+idx*58, "%04d" % down, c, 2)
