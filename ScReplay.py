...
class ScReplay(ScBase):
	...
	def __init__(self, pCTX, pRender, pWanem):
		...
		self.ptDef.insert(1, self.CreateTocuhDef("BtGroupPrev",   470,       264,         80, 50, self.BtHandler))
		self.ptDef.insert(2, self.CreateTocuhDef("BtGroupNext",   388,       264,         80, 50, self.BtHandler))
	...
	def UpdateGroup(self, vec, forceClear = False):
		...
		if (self.groupPageIdx * 5 + self.groupFocusIdx) == 0 and vec == -1:
			return
		if (self.groupPageIdx * 5 + self.groupFocusIdx) == (self.groupNr - 1) and vec == 1:
			return
		...
		if isPageSwitch:
			...
			file = self.groupList[idx]
			self.pRender.fb.putstr(12, 100 + 30 * (self.groupFocusIdx % 5), "> %s" % file[0:8], self.pRender.W, 2)
	...
	def UpdateCategory(self, vec, forceClear = False):
		...
		if (self.catPageIdx * 5 + self.catFocusIdx) == 0 and vec == -1:
			return
		if (self.catPageIdx * 5 + self.catFocusIdx) == (self.catNr - 1) and vec == 1:
			return
		...
		if isPageSwitch:
			...
			file = self.catList[idx]
			self.pRender.fb.putstr(202, 100 + 30 * (self.catFocusIdx % 5), "> %s" % file[0:8], self.pRender.W, 2)
