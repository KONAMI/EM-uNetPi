...
class ScSetting(ScBase):
	...
	def __init__(self, pCTX, pRender, pWanem):
		...
		self.ptDef.append(self.CreateTocuhDef("BtMenu",   468,   29,         62, 42, self.BtHandler))
		self.ptDef.append(self.CreateTocuhDef("BtTab0",   466,   96+45*0,    84, 42, self.BtHandler))
		self.ptDef.append(self.CreateTocuhDef("BtTab1",   466,   96+45*1,    84, 42, self.BtHandler))
		self.ptDef.append(self.CreateTocuhDef("BtTab2",   466,   96+45*2,    84, 42, self.BtHandler))
		self.ptDef.append(self.CreateTocuhDef("BtTab3",   466,   96+45*3,    84, 42, self.BtHandler))
		self.ptDef.append(self.CreateTocuhDef("BtTab4",   466,   96+45*4,    80, 42, self.BtHandler))
		self.ptDef.append(self.CreateTocuhDef("BtNaptL",  366,   135,        80, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtNaptR",  116,   135,        80, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtWanemL", 366,   135,        80, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtWanemR", 116,   135,        80, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtDhcpL",  366,   270,        80, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtDhcpR",  116,   270,        80, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtDhcpC",  254,   272,       100, 44, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtReboot", 318,   242,       100, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtShutdown", 184,   242,       100, 48, self.BtHandler, None, False))
	
		self.ptDef.append(self.CreateTocuhDef("BtApL",      366,   240,      36, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtApR",      241,   240,      36, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtChannelL", 200,   240,      36, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtChannelR", 75,    240,      36, 48, self.BtHandler, None, False))

		self.ptDef.append(self.CreateTocuhDef("BtMacReport0",    113,  148,  38, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtMacTmpReport0", 77,   148,  38, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtMacReport1",    113,  210,  38, 48, self.BtHandler, None, False))
		self.ptDef.append(self.CreateTocuhDef("BtMacTmpReport1", 77,   210,  38, 48, self.BtHandler, None, False))
	...
	def UpdateApMode(self, vec):
		...
		self.pRender.fb.putstr(168-5,   154+95-12, self.apModeLabelsT[self.currentApMode], c, 2)
		self.pRender.fb.putstr(168,     154+95+12, self.apModeLabelsB[self.currentApMode], c, 2)

		if vec != 0:
			self.currentChannel = 0
			self.UpdateApChannel(0, True)
			cmd = "cp /etc/wanem/tpl/" + str(self.currentApMode) + ".prop /etc/wanem/apmode.prop"
			try:
	...
	def UpdateApChannel(self, vec, forceUpdate = False):
		...
		self.currentChannel = (self.currentChannel + vec + channelsLen) % channelsLen
		c = self.pRender.N
		self.pRender.fb.draw.rect(c, Rect(166+175, 236, 74, 44), 0)

		c = self.pRender.ConvRgb(0.26,0.1,0.9)
		if self.currentApMode == 0:
			self.pRender.fb.putstr(168+175-18, 154+92, self.apChannelLabelsG[self.currentChannel], c, 3)
		else:
			self.pRender.fb.putstr(168+175-18, 154+92, self.apChannelLabelsA[self.currentChannel], c, 3)

		if vec != 0 or forceUpdate:
			cmd = "cp /etc/wanem/tpl/" + str(self.currentChannel) + ".prop /etc/wanem/apchannel.prop"
			try:
				subprocess.check_call(cmd.strip().split(" "))
	...
	def UpdateNatpMode(self, vec):
		self.currentNaptMode = (self.currentNaptMode + vec + len(self.naptModeLabels)) % 2
		c = self.pRender.N
		self.pRender.fb.draw.rect(c, Rect(210, 140+95+10-105, 160, 24), c)		
		c = self.pRender.ConvRgb(0.26,0.1,0.9)
		self.pRender.fb.putstr(225, 154+95-105, self.naptModeLabels[self.currentNaptMode], c, 2)
		self.pRender.fb.putstr(144, 294-105, 'NAPT Setting change will be applied after reboot.', self.pRender.R, 1) 
	...
	def UpdateWanemMode(self, vec):
		self.currentWanemMode = (self.currentWanemMode + vec + len(self.wanemModeLabels)) % 2
		c = self.pRender.N
		self.pRender.fb.draw.rect(c, Rect(210, 140+95+10-105, 160, 24), c)		
		c = self.pRender.ConvRgb(0.26,0.1,0.9)
		self.pRender.fb.putstr(225, 154+95-105, self.wanemModeLabels[self.currentWanemMode], c, 2)

		if vec != 0:
			cmd = "cp /etc/wanem/tpl/" + str(self.currentWanemMode) + ".prop /etc/wanem/wanemmode.prop"
			try:
				subprocess.check_call(cmd.strip().split(" "))
	...
	def	RenderTab0(self):
		...
		self.pRender.fb.putstr(130, 128 + 32 * 0, "SSID : wanem-" + self.GetSelfId(), self.pRender.W, 2)
		self.pRender.fb.putstr(130, 128 + 32 * 1, "PASS : wanem-" + self.GetSelfId(), self.pRender.W, 2)
		...
		self.pRender.fb.putstr(144, 294, 'AP Setting change will be applied after reboot.', self.pRender.R, 1) 
	...
	def	RenderTab2(self):
		...
		self.pRender.fb.putstr(130, 128 + 32 * 0, "WAN IP : " + wanIp, self.pRender.W, 2)
		if self.pCTX.lanMode == self.pCTX.LAN_MODE_WLAN:
			self.pRender.fb.putstr(130, 128 + 32 * 1, "LAN IP : " + lanIp, self.pRender.W, 2)
		else:
			self.pRender.fb.putstr(130, 128 + 32 * 1, "LAN IP : ", self.pRender.W, 2)
			self.pRender.fb.putstr(130+108, 128 + 28 * 1,       "WIRELESS > " + lanIp,  self.pRender.W, 1)
			self.pRender.fb.putstr(130+108, 128 + 28 * 1 + 16,  "WIRED    > " + lan2Ip, self.pRender.W, 1)	
		
		self.pRender.fb.putstr(130, 128 + 32 * 2, "MAC    : " + hwaddr, self.pRender.W, 2)

		self.RenderHeaderLabel(220,  "       NEP REVISION")
		self.pRender.fb.putstr(130, 220 + 47, "REV    : " + self.pCTX.revision, self.pRender.W, 2)
	...
	def RenderClientInfo(self, idx, total, ip, mac, clientHostname, isSended):
		...
		self.pRender.fb.putstr(120, baseY, "Client " + str(idx+1) +  "/" + str(total) + " : " + clientHostname, self.pRender.W, 1)		
		self.pRender.fb.putstr(120, baseY + 24 * 0 + 14, "IP :" + ip, self.pRender.W, 2)
		self.pRender.fb.putstr(120, baseY + 24 * 1 + 14, "MAC:" + mac, self.pRender.W, 2)
		...
		if self.pCTX.apiStatus == 0: 							
			if isSended == 0:
				...
				self.pRender.fb.draw.rect(c, Rect(382 + 40, baseY + 10, 38, 44), 0)
				c = self.pRender.ConvRgb(0.26,0,0.5)
				...
				self.pRender.fb.draw.rect(c, Rect(382 + 40, baseY + 54, 38, 4), 0)
	...
	def UpdateDhcpList(self, vec, isCleanup = True):
		if isCleanup == True:
			self.pRender.fb.draw.rect(self.pRender.N, Rect(120, 124, 340, 128), 0)

		if self.currentClientTotal > 1:
			block = self.currentClientTotal
			if (block % 2) == 1:
				block += 1
			self.currentClientIdx = (self.currentClientIdx + vec + block) % block
			
		if self.currentClientIdx < self.currentClientTotal:
			info = self.dhcpClientInfo[self.currentClientIdx]
			self.RenderClientInfo(info[0], self.currentClientTotal, info[1], info[2], info[3], info[4])
	...
	def SendDhcpClientInfo(self, odd, isTmp):			
		...
		LogReporter.ReportDhcpClientInfo(self.pCTX, info, "wanem-" + self.GetSelfId(), isTmp)
	...
	def	RenderTab4(self):
		...
		self.pRender.fb.putstr(130 + 80, 300, self.pCTX.copyright, self.pRender.W, 1)
					...
					if state == "active":
						...
						for node in self.dhcpClientInfo:
							if node[2] == mac:
								...
								break
						if isExist == False:
							self.dhcpClientInfo.append([len(self.dhcpClientInfo), ip, mac, clientHostname, 0])
	...
	def Start(self):
		...
		try:
			self.currentWanemMode = int(subprocess.check_output(cmd.strip().split(" ")).replace('\n',''))
		except subprocess.CalledProcessError:
			self.currentWanemMode = 0
			
		cmd = "cat /etc/wanem/apchannel.prop"			
		try:
			self.currentChannel = int(subprocess.check_output(cmd.strip().split(" ")).replace('\n',''))
		except subprocess.CalledProcessError:
			self.currentChannel = 1

		cmd = "cat /etc/wanem/apmode.prop"			
		try:
			self.currentApMode = int(subprocess.check_output(cmd.strip().split(" ")).replace('\n',''))
		except subprocess.CalledProcessError:
