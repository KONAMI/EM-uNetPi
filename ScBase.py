...
class ScBase(object):
	...
	def Start(self):		
		...
		self.pRender.fb.draw.rect(c, Rect(0, 0, self.pRender.fb.xres-1, self.pRender.fb.yres-1), 1)
		self.pRender.fb.draw.rect(c, Rect(2, 2, 4, 4), 0)
		self.pRender.fb.draw.rect(c, Rect(2, self.pRender.fb.yres-6, 4, 4), 0)
		self.pRender.fb.draw.rect(c, Rect(self.pRender.fb.xres-6, 2, 4, 4), 0)
	...
	def CallTouchFunc(self, x, y, isOnDown):
		for idx in range(0, len(self.ptDef)):
			...
			if pt[7] != True:
				continue
			if x >= (pt[1]-pt[3]) and x <= pt[1] and y > pt[2] and y < (pt[2]+pt[4]):
				if isOnDown == True:
					if pt[5] is not None:
						pt[5](pt[0])
				else:
